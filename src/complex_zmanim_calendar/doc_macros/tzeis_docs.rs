...
macro_rules! tzeis_degrees_basedon_doc {
    ($degrees:expr, $minutes:expr) => {
        concat!(
            ...
            stringify!($degrees),
            "&deg; below the western geometric horizon after sunset. This
            calculation is based on the position of the sun ",
            stringify!($minutes),
            " minutes after sunset in Jerusalem around the equinox / equilux,
            which calculates to ",
            stringify!($degrees),
...
macro_rules! tzeis_degrees_doc {
    ($degrees:expr) => {
        concat!(
            "Returns *tzeis* (dusk) calculated when the sun is ",
            stringify!($degrees),
            "&deg; below the western geometric horizon after sunset."
        )
    };
}
...
macro_rules! tzeis_minutes_basedon_doc {
    ($minutes:expr, $mil_len:expr) => {
        concat!(
            ...
            stringify!($minutes),
            " minutes after sunset. This time is based on the time to walk the
